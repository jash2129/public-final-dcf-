...
export default function UserDetail() {
  ...
  console.log("Binding state to UI:", { orders, documents, invoices });
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 text-slate-100">
      ...
      <button 
        onClick={() => navigate('/admin/orders')}
        className="flex items-center gap-2 text-slate-400 hover:text-slate-100 transition-colors text-sm font-bold bg-slate-900/40 hover:bg-slate-900/80 px-4 py-2.5 rounded-xl border border-slate-800/80 cursor-pointer"
      >
        <ArrowLeft className="h-4 w-4" /> Back to Orders
      </button>

      {/* Main Container - Premium Obsidian Styling */}
      <div className="bg-slate-950 border border-slate-800 rounded-2xl shadow-2xl p-6 space-y-6">
        
        {/* Profile Header Block */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pb-6 border-b border-slate-800">
          
          {/* User Info Sidebar - contrast bg-slate-900/50 */}
          <div className="md:col-span-1 bg-slate-900/50 border border-slate-800/60 rounded-xl p-5 flex flex-col items-center text-center space-y-4 shadow-inner">
            <div className="h-20 w-20 bg-slate-950 border border-slate-800 rounded-full flex items-center justify-center font-bold text-2xl text-slate-100 shadow-inner flex-shrink-0">
              {user.avatar ? (
            ...
            <div>
              <h2 className="text-xl font-black text-white tracking-tight leading-tight">{user.name}</h2>
              <span className="inline-block mt-2 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-widest bg-brand/20 text-brand border border-brand/30 rounded-full">
                {user.role}
              </span>
            </div>

            <div className="w-full border-t border-slate-800/80 pt-4 space-y-2 text-xs text-left">
          ...
          <div className="md:col-span-2 flex flex-col justify-center space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div>
                <span className="text-xs text-slate-400 font-bold uppercase tracking-wider block">WhatsApp / Phone</span>
                <span className="text-sm text-white font-medium">{user.whatsapp_number || user.phone || 'N/A'}</span>
              </div>

              {user.company_name && (
                <div>
                  <span className="text-xs text-slate-400 font-bold uppercase tracking-wider block">Company Name</span>
                  <span className="text-sm text-white font-medium">{user.company_name}</span>
                </div>
              )}

              {user.gstin && (
                <div>
                  <span className="text-xs text-slate-400 font-bold uppercase tracking-wider block">GSTIN</span>
                  <span className="text-sm text-white font-medium font-mono">{user.gstin}</span>
                </div>
              )}

              {user.address && (
                <div className="sm:col-span-2">
                  <span className="text-xs text-slate-400 font-bold uppercase tracking-wider block">Billing Address</span>
                  <span className="text-sm text-white font-medium leading-relaxed block mt-1">{user.address}</span>
                </div>
              )}
            ...
            {[
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 pb-4 text-sm transition-all relative cursor-pointer ${
                    isActive 
                      ? 'border-b-2 border-brand-500 text-brand-500 font-semibold' 
              ...
              <div className="space-y-4">
                {orders.length > 0 ? (
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="border-b border-slate-800 bg-slate-900/40 text-slate-400 font-medium">
                          ...
                          <th className="px-6 py-4 text-xs font-semibold uppercase tracking-wider">Service</th>
                      ...
                      <tbody className="divide-y divide-slate-800">
                        {orders.map((order) => (
                          <tr key={order.id} className="hover:bg-slate-900/50 border-b border-slate-800 transition-colors">
                            <td className="px-6 py-4 text-sm font-bold text-white">{order.id}</td>
                            <td className="px-6 py-4 text-sm font-medium text-slate-300">{order.service}</td>
                  ...
                  <EmptyState 
                    ...
                    className="border-slate-800 bg-slate-900/10"
                  />
                )}
              </div>
            )}
              ...
              <div className="space-y-4">
                {documents.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {documents.map((doc) => (
                      <div key={doc.id} className="p-4 bg-slate-900/30 rounded-xl border border-slate-800 hover:border-slate-700 transition-all flex items-center justify-between gap-4">
                        <div className="flex items-center gap-3 min-w-0">
                          <div className="h-10 w-10 bg-slate-900 rounded-xl flex items-center justify-center border border-slate-800 shrink-0 text-slate-300">
                            {doc.type === 'pdf' ? <FileText className="h-5 w-5 text-red-400" /> : <FileCode className="h-5 w-5 text-blue-400" />}
                          </div>
                          <div className="min-w-0">
                            <p className="text-sm font-bold text-slate-200 truncate" title={doc.name}>{doc.name}</p>
                            <p className="text-xs text-slate-500 mt-0.5 font-mono">{doc.size} • {doc.date}</p>
                          </div>
                        </div>
                        <a 
                          href={`/api/documents/admin/${doc.id}/file`} 
                          download
                          className="p-2.5 text-slate-400 hover:text-white bg-slate-900 hover:bg-slate-800 rounded-lg transition-all border border-slate-800 shrink-0 cursor-pointer"
                        >
                ...
                {invoices.length > 0 ? (
                  <div className="overflow-x-auto rounded-xl border border-slate-800 bg-slate-950">
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="border-b border-slate-800 bg-slate-900/40 text-slate-400 font-medium">
                          <th className="px-6 py-4 text-xs font-semibold uppercase tracking-wider">Invoice ID</th>
                          <th className="px-6 py-4 text-xs font-semibold uppercase tracking-wider">Date</th>
                          <th className="px-6 py-4 text-xs font-semibold uppercase tracking-wider">Service</th>
                          <th className="px-6 py-4 text-xs font-semibold uppercase tracking-wider">Amount</th>
                          <th className="px-6 py-4 text-xs font-semibold uppercase tracking-wider text-right">Download</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-800">
                        {invoices.map((invoice) => (
                          <tr key={invoice.id} className="hover:bg-slate-900/50 border-b border-slate-800 transition-colors">
                            <td className="px-6 py-4 text-sm font-bold text-white">{invoice.id}</td>
                            <td className="px-6 py-4 text-sm text-slate-400">{invoice.date}</td>
                            <td className="px-6 py-4 text-sm font-medium text-slate-300">{invoice.service}</td>
                            <td className="px-6 py-4 text-sm font-bold text-white">{invoice.amount}</td>
                            <td className="px-6 py-4 text-right">
                              <button 
                                onClick={() => window.open('/api/admin/invoices/' + invoice.orderId + '/download', '_blank')}
                                className="bg-brand bg-brand-500 hover:bg-brand-600 text-white font-bold px-4 py-2 rounded-lg text-xs transition-all shadow-md inline-flex items-center gap-1.5 cursor-pointer"
                              >
