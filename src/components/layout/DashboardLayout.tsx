...
export default function DashboardLayout() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex font-sans text-dark selection:bg-brand/30 selection:text-dark">
      {/* Mobile Backdrop */}
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div 
      ...
      <aside className={`
        ...
        bg-white border-r border-slate-200 flex flex-col shadow-premium
      `}>
        <div className={`h-20 flex items-center ${isSidebarCollapsed ? 'justify-center px-4' : 'px-6 justify-between'} border-b border-slate-100 bg-white/50 backdrop-blur-md`}>
          ...
          <div className="flex items-center gap-2">
            {!isSidebarCollapsed && (
              <button 
                ...
                className="p-1.5 text-slate-400 hover:text-dark hover:bg-slate-100 rounded-lg transition-colors hidden md:block"
              >
            ...
            <button 
              onClick={closeMobileMenu} 
              className="p-1.5 text-slate-400 hover:text-dark hover:bg-slate-100 rounded-lg transition-colors md:hidden"
            >
              <X className="h-5 w-5" />
            </button>
          </div>
        ...
        {(!user.role || user.role === 'user') && (
          <div className={`py-4 ${isSidebarCollapsed ? 'px-3' : 'px-6'}`}>
            {!isSidebarCollapsed && <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3 px-2">Main Menu</p>}
            <nav className="space-y-1.5">
              {navItems.map((item) => {
                ...
                return (
                  ...
                  >
                    {isActive && (
                      <span className="absolute left-0 top-1/4 bottom-1/4 w-[3px] bg-brand rounded-r" />
                    )}
        ...
        {(user.role === 'admin' || user.role === 'super_admin') && (
          <div className={`py-4 border-t border-slate-100 ${isSidebarCollapsed ? 'px-3' : 'px-6'}`}>
            {!isSidebarCollapsed && <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3 px-2">Admin Portal</p>}
            <nav className="space-y-1.5">
              {adminNavItems.map((item) => {
                ...
                return (
                  ...
                  >
                    {isActive && (
                      <span className="absolute left-0 top-1/4 bottom-1/4 w-[3px] bg-dark rounded-r" />
                    )}
                    <item.icon className={`h-5 w-5 ${isActive ? 'text-dark' : 'text-slate-400'}`} />
        ...
        <div className={`py-4 border-t border-slate-100 ${isSidebarCollapsed ? 'px-3' : 'px-6'}`}>
          {!isSidebarCollapsed && <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3 px-2">Business Tools</p>}
        ...
        <div className={`mt-auto border-t border-slate-100 ${isSidebarCollapsed ? 'p-3' : 'p-4'}`}>
          {!isSidebarCollapsed && (
            <div className="bg-slate-50 rounded-2xl p-4 mb-4 border border-slate-100">
              <div className="flex items-center gap-3 mb-3">
                <div className="bg-brand-lightest p-2 rounded-lg">
                  <LifeBuoy className="h-5 w-5 text-dark" />
                </div>
                <div>
                  <p className="text-sm font-bold text-dark">Need Help?</p>
                  <p className="text-xs text-slate-500">24/7 Expert Support</p>
                </div>
              </div>
              <button 
                onClick={() => navigate('/contact')}
                className="w-full bg-white border border-slate-200 text-dark text-xs font-bold py-2 rounded-lg hover:bg-slate-100 transition-colors cursor-pointer"
              >
                Contact Support
              </button>
            </div>
          ...
          {/* Dedicated Explicit Sign Out Item */}
          <button
            onClick={handleLogout}
            title={isSidebarCollapsed ? "Sign Out" : ""}
            className={`w-full flex items-center mb-2 ${isSidebarCollapsed ? 'justify-center py-2.5' : 'gap-3 px-4 py-2.5'} rounded-xl text-sm font-bold text-slate-500 hover:bg-red-50 hover:text-red-600 transition-all group/logout cursor-pointer`}
          >
            <LogOut className="h-5 w-5 text-slate-400 group-hover/logout:text-red-500 transition-colors" />
            {(!isSidebarCollapsed || isMobileMenuOpen) && <span>Sign Out</span>}
          </button>

          {/* User Profile display card (Informational only, no trigger action) */}
          <div 
            title={isSidebarCollapsed ? `${user.name} (${user.email})` : ""}
            className={`flex items-center rounded-xl transition-colors group ${isSidebarCollapsed ? 'justify-center py-2' : 'px-2 py-2 gap-3 bg-slate-50 border border-slate-100'}`}
          >
            <div className="h-10 w-10 bg-dark text-white rounded-full flex items-center justify-center font-bold text-sm shadow-sm transition-transform group-hover:scale-105 flex-shrink-0">
      ...
      <div className="flex-1 flex flex-col min-h-screen transition-all duration-300">
        {/* Top Header */}
        <header className="bg-white/70 backdrop-blur-xl border-b border-slate-200/50 h-20 flex items-center justify-between px-8 sticky top-0 z-10 transition-all duration-300">
          ...
          <div className="flex items-center gap-6">
            ...
            <button className="relative p-2 text-slate-400 hover:text-dark transition-colors rounded-full hover:bg-slate-50">
